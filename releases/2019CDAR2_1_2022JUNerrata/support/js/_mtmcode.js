...
var MTMCodeFrame = "code", MTMenuFrame = "menu", MTMTableWidth = "100%", MTMenuImageDirectory = "../../support/graphics/";
...
function MTMSubAction(SubItem) {
	...
	if(MTMEmulateWE || SubItem.url == "" || !SubItem.expanded) {
		...
		//Changed by Michael Krot, 11-23 from !MTMUA.resolveURL(SubItem.url, true) to MTMUA.resolveURL(SubItem.url, true) 
	...
	if(MTMUA.MTMable && !MTMUpdating) {
		...
		if((MTMClickedItem || MTMTrackedItem) && !(MTMUA.browserType == "NN" && MTMUA.majVersion == 3)) {
			...
			if(document.layers && MTMUA.menuFrame.scrollbars) {
				...
				// Alexander Henket: Fix for Apple Safari. Change document.all[...] to document.getElementsByName(...).item(0);
                /*
                MTMyval = MTMGetYPos(MTMUA.document.all[MTMItemName]);
				MTMUA.document.all[MTMItemName].focus();
				*/
				MTMyval = MTMGetYPos(MTMUA.document.getElementsByName(MTMItemName).item(0));
				MTMUA.document.getElementsByName(MTMItemName).item(0).focus();
...
function MTMappendElement() {
	...
	if(arguments[0] == 'head') {
	    // Alexander Henket: Fix for null pointer exception in Apple Safari
        if (this.head == null) { this.head = document.createElement('head'); }
...
function MTMDisplayItem(item, last) {
	...
	if(item.submenu) {
		if(MTMEmulateWE) {
			if(item.url != "") {
				MTMOutputString += '</a>' + MTMakeLink(item, false, false, true); //this is important			
...
function MTMakeLink(thisItem, voidURL, addName, addTitle, clickEvent, mouseOverEvent, mouseOutEvent) {
	var tempString = '<a class="lhnMenuItem" href="' + (voidURL ? 'javascript:;' : MTMUA.resolveURL(thisItem.url)) + '" ';
